...
const init = () => {
  ...
  let listImg = dataImages.map(elt => createImage(elt));
  listImg.forEach(elt => elt.addEventListener("mouseover",details));
  let mur = document.querySelector('#mur');
  listImg.forEach(elt => mur.appendChild(elt));
...
const filtreImages = event => {
  let listSelec = document.querySelectorAll('#mur img');
  let murList = [...listSelec];
  let value = document.querySelector('#filtre').value.toLowerCase();
  console.log(murList);
  murList.filter(elt => !(elt.title.toLowerCase().includes(value))).forEach(elt2 => elt2.style.display = 'none');
  murList.filter(elt => elt.title.toLowerCase().includes(value)).forEach(elt2 => elt2.style.display = 'initial');
}
